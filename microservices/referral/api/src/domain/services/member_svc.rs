...
impl MemberService {
    ...
    pub async fn create_referral(user_id: Uuid, event: MemberCreatedEvent) -> Result<ReferralEvent, Status> {
        ...
        let form_data: member::Model = member::Model {
            ...
            hierarchy: if referrer.is_some() { referrer.unwrap().hierarchy + 1 } else { 0 },
            active: true,
            ...
            Ok(opt) => match opt {
                ...
                Some(m) => {
                    let _ = MemberMutation::update_member(member::Model {
                        member_type, level, active, description,
                        updated_at: Local::now().naive_local(),
                        version: m.version + 1, ..m
                    }).await;
                    Ok(ReferralEvent::Updated)
                }
            }
            Err(e) => Err(Status::internal("Failed to"))
        }
    }
}
