...
use shared::utils::{to_uuid, AuthError, CustomResponse, CustomResponseBuilder, ValidatedPath};
use shared::utils::{CustomError, ValidatedJson};

pub fn sessions_routes() -> Router {
  Router::new().route("/sessions", post(authenticate)).route("/sessions/:id", delete(unauthenticate))
}
...
async fn unauthenticate(ValidatedPath(id): ValidatedPath<String>) -> Result<CustomResponse<()>, CustomError> {
  ...
  let _ = token_refresh_svc::remove_access_token(user_id.clone()).await?;
