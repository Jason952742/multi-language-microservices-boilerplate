...
impl AccountServices {
    ...
    pub async fn create_event(account_id: &Uuid, user_id: &Uuid, transaction_id: &Uuid) -> eventsource::Model {
        let account = Account::new(&account_id);
        let cmd = AccountCommand::OpenAccount {
    ...
    pub async fn deposit_event(account: &Account, payment: Payment)-> (eventsource::Model, Decimal) {
        let cmd = AccountCommand::DepositMoney {
