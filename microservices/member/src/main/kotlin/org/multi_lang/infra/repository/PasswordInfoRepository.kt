...
class PasswordInfoRepository : JasPanacheRepository<PasswordInfo> {
    ...
    fun save(model: org.multi_lang.domain.entity.PasswordInfo): Uni<org.multi_lang.domain.entity.PasswordInfo> = persist(model.apply { password.encrypt() })

    fun verify(loginCreds: String, password: String): Uni<org.multi_lang.domain.entity.PasswordInfo?> = find(
        "loginCreds = :loginCreds and password = :password",
        mapOf("loginCreds" to loginCreds, "password" to password.encrypt())
    ).firstResult()

    fun findByLoginCreds(loginCreds: String): Uni<org.multi_lang.domain.entity.PasswordInfo?> = find("loginCreds", loginCreds).firstResult()
    ...
    suspend fun searchAndCount(q: JasQuery): Uni<JasPaging<PasswordInfo>> {
        ...
        val total = p
            .count()
            .awaitSuspending()
        val items = p.range<org.multi_lang.domain.entity.PasswordInfo>(q.offset, q.last())
            .list<org.multi_lang.domain.entity.PasswordInfo>()
            .awaitSuspending()
